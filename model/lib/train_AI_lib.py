...
def convertAlex(dictPath = "saved/splitData", inPath = "data/working-wheat-data/train", oPath = 'saved/alexFeatures'):
    """
    Takes the three dictionaries generated by splitData(), takes the images from inpath, and then convertes them into files in oPath
    Arguments:
        dictPath: Path containing the dictionaries generated by splitData()
        inPath  : Path images one wants to turn into alex net features
        oPath   : Path that the features are output to. features are saved by torch.save() under the same name as the original img without the '.jpg'
    """
    ...
    for data in [trainData, valData, testData]:
        if data == trainData: print("Converting Training Images"  )
        if data == valData  : print("Converting Validation Images")
        if data == testData : print("Converting Testing Images"   )
        for i, (img, _, imgName, _) in enumerate(data):
            img = alexnet.features(torch.unsqueeze(img, 0).cuda())
            torch.save(img.detach().cpu(), oPath+'/'+imgName.split('.jpg')[0])
            if i%100 == 0: print("Converted {:.2f}%".format(100*i/len(data)))
